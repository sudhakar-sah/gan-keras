...
def generator(g_input):
	...
	x = Dense(dim*dim*256, input_shape = input_shape, init=gln)(g_input)
	...
	x = UpSampling2D()(x)
	x = Conv2DTranspose(filters=256, kernel_size=5, padding='same')(x)
	x = BatchNormalization(momentum = 0.9)(x)
	x = Activation('relu')(x)
	...
	x = Conv2DTranspose(filters=channels, kernel_size=5, padding='same')(x)
	x = Activation('sigmoid')(x)
...
img_dim = np.zeros((img_height,img_width,channels))
...
def face_data():
	...
	X_train = np.zeros((len(faces), img_height, img_width, channels))
...
def desc_model_pretrain(desc_model, size_tr_data = 10000):
	...
	# print (XT.shape, generator_images.shape)
...
train_for_n(gen_model, desc_model, gan_model, epochs=100, plt_freq=5,batch_size=32)
